...
(defun hhamud/get-current-week-string ()
  "Return the current week in the format 'mar01-07', starting from the Monday of the current week until the following Sunday."
  (let* ((time (current-time))
         ...
         (current-dow (nth 6 decoded-time))
         (days-to-monday (- current-dow 1))
         (monday-time (time-subtract time (days-to-time (* days-to-monday 86400))))
         (monday-day (nth 3 (decode-time monday-time)))
         (sunday-day (+ monday-day 6))
         (week-start-string (downcase (format-time-string "%b%02d" monday-time)))
         (week-end-string (format "%02d" sunday-day)))
